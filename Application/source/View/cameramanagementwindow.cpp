...
void CameraManagementWindow::OnActionButtonClick()
{
    try {
        ...
        if ((m_windowController->GetDatabase().ToCameraKey().FindByKey(m_keysMap[ui->cameraKeyComboBox->currentText()]).GetID() != 0
            && m_windowController->GetDatabase().ToCameraKey().FindByKey(m_keysMap[ui->cameraKeyComboBox->currentText()]).GetKey()
            != m_windowController->GetDatabase().ToCameraKey().FindByID(ui->cameraIDLineEdit->text().toInt()).GetKey())
            && m_windowController->GetCurrentMode() == CameraManagementWindowController::CameraManagementMode::UPDATE)
...
void CameraManagementWindow::closeEvent(QCloseEvent* event)
{
    m_selectedCamera = nullptr;
    ui->cameraIDLineEdit->setText("");
    ui->cameraLocationLineEdit->setText("");
    ui->cameraNameLineEdit->setText("");
    ui->cameraTypeComboBox->setCurrentIndex(0);
    ui->cameraKeyComboBox->setCurrentIndex(0);
    event->accept();
}
